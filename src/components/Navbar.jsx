...
export default function Navbar() {
  ...
  const { handleName,getData,name } = useContext(ContactContext)
  useEffect(()=>{
    ...
    <>
      ...
      <nav className=" h-15 fixed top-0 w-[100vw] text-[13px] font-medium flex justify-between items-center bg-black">
        ...
        <div className="text-gray-400 flex gap-5 mr-3 items-center">
          {localStorage.getItem('authtoken') && <NavLink className={location.pathname === `/${name.trim()}` ? "text-white " : ""} to={`/${name}`} end>HOME</NavLink>}
          {localStorage.getItem('authtoken') && <NavLink className={location.pathname === `/contacts/${name.trim()}` ? "text-white" : ""} to={`/contacts/${name}`} end>MY CONTACTS</NavLink>}
      ...
      {localStorage.getItem('authtoken') && <div id='icon' className='flex text-gray-400 text-[14px] font-medium flex-col absolute p-8 top-14 gap-8 initial bg-black/70 h-50 right-0'>
        <NavLink className={location.pathname === `/${name}` ? "text-white" : ""} to={`/${name.trim()}`} end>HOME</NavLink>
        <NavLink className={location.pathname === `/contacts/${name}` ? "text-white" : ""} to={`/contacts/${name.trim()}`} end>MY CONTACTS</NavLink>
        <NavLink to='/login' end><button onClick={() => { localStorage.removeItem('authtoken') }} className="">LOGOUT</button></NavLink>
      </div>
      }
